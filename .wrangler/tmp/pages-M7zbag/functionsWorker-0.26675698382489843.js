...
// api/offers/admantum-feed.js
var ADMANTUM_APP_ID = "52189";
async function onRequestGet5(context) {
  ...
  const feedUrl = `https://admantum.com/api/v3/offers/?appid=${encodeURIComponent(ADMANTUM_APP_ID)}&uid=${encodeURIComponent(me.id)}&device=${encodeURIComponent(device)}` + (ip ? `&ip=${encodeURIComponent(ip)}` : "");
  try {
...
__name(normalizeAdmantumOffer, "normalizeAdmantumOffer");
...
// api/offers/admantum-postback.js
function ok() {
...
async function handlePostback(request, env) {
  ...
  const amountRaw = params.virtual_currency;
...
// api/offers/cpalead-postback.js
var PROVIDER = "cpalead";
...
async function handlePostback2(request, env) {
  ...
  const coinsEarned = Math.max(0, Math.round(payout * COINS_PER_DOLLAR2));
  const existing = await env.DB.prepare(
    "SELECT id FROM offer_completions WHERE provider = ? AND transaction_id = ?"
  ).bind(PROVIDER, leadId).first();
  if (existing) {
  ...
  const update = await env.DB.prepare(
    `UPDATE users
     SET coins = coins + ?,
         ...
         total_earning = total_earning + ?
     WHERE id = ?`
  ).bind(coinsEarned, payout, userId).run();
  ...
  ).bind(PROVIDER, userId, params.campaignId || null, leadId, payout, coinsEarned).run();
  return jsonResponse({ status: "credited", coins: coinsEarned }, 200);
}
__name(handlePostback2, "handlePostback");
async function onRequestGet7(context) {
  return handlePostback2(context.request, context.env);
}
...
async function handlePostback4(request, env) {
  ...
  if (!secretKey) {
    return fail2("Server not configured (missing GEMIAD_SECRET_KEY).", 500);
  }
  if (!hash || !userId || !offerId || !txId) {
    return fail2("Missing required parameters.", 400);
  }
  if (!Number.isFinite(reward)) {
    return fail2("Missing or invalid parameters.", 400);
  }
  const expectedHash = await sha256Hex(userId + offerId + txId + secretKey);
  if (hash !== expectedHash) {
    return fail2("Invalid hash.", 403);
  }
  ...
  if (status === "completed") {
    ...
    if (!update.success || update.meta.changes === 0) {
      return fail2("User not found.", 404);
    }
    await env.DB.prepare(
      `INSERT INTO offer_completions (provider, user_id, offer_id, transaction_id, payout, coins_earned, status, created_at)
  ...
  if (status === "rejected") {
    ...
    await env.DB.prepare(`UPDATE offer_completions SET status = 'reversed' WHERE id = ?`).bind(original.id).run();
    return approved();
  }
...
async function onRequestGet18(context) {
  ...
  const feedUrl = `https://wall.taskwall.io/api/?app_id=${encodeURIComponent(TASKWALL_APP_KEY)}&userid=${encodeURIComponent(me.id)}` + (os ? `&os=${encodeURIComponent(os)}` : "") + (country ? `&country=${encodeURIComponent(country)}` : "");
  try {
    const res = await fetch(feedUrl, {
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36 earnbangla-offerwall-proxy"
...
__name(normalizeTaskwallOffer, "normalizeTaskwallOffer");
...
async function handleCompleted(p, env) {
  ...
  ).bind(PROVIDER2, p.txid).first();
  if (existing) {
    return jsonResponse2({ status: "duplicate" }, 200);
  }
  const update = await env.DB.prepare(
    `UPDATE users
     SET coins = coins + ?,
         ...
         total_earning = total_earning + ?
     WHERE id = ?`
  ).bind(points, payout, p.userId).run();
  if (!update.meta || update.meta.changes === 0) {
    return jsonResponse2({ status: "error", message: "User not found." }, 404);
  }
  await env.DB.prepare(
    `INSERT INTO offer_completions
       (provider, user_id, offer_id, transaction_id, payout, coins_earned, status, created_at)
     VALUES (?, ?, ?, ?, ?, ?, 'credited', datetime('now'))`
  ).bind(PROVIDER2, p.userId, p.campaignId, p.txid, payout, points).run();
  return jsonResponse2({ status: "credited", coins: points }, 200);
}
__name(handleCompleted, "handleCompleted");
async function handleRejected(p, env) {
  ...
  if (!original) {
    ...
    return jsonResponse2({ status: "ignored", reason: "no matching credited txn" }, 200);
  }
  ...
  if (original.user_id !== p.userId) {
    ...
    return jsonResponse2({ status: "ignored", reason: "user mismatch" }, 200);
  }
  await env.DB.prepare(
    `UPDATE users
     ...
     WHERE id = ?`
  ).bind(original.coins_earned, original.payout, p.userId).run();
  await env.DB.prepare(
    `INSERT INTO offer_completions
       (provider, user_id, offer_id, transaction_id, payout, coins_earned, status, created_at)
     VALUES (?, ?, ?, ?, ?, ?, 'reversed', datetime('now'))`
  ).bind(PROVIDER2, p.userId, p.campaignId, p.txid, -original.payout, -original.coins_earned).run();
...
async function handlePostback11(request, env) {
  ...
  if (!secret) {
    return jsonResponse2({ status: "error", message: "Server not configured." }, 500);
  }
  const expectedHash = await sha256Hex2(p.userId + p.campaignId + p.txid + secret);
  if (!timingSafeEqual2(p.hash, expectedHash)) {
    return jsonResponse2({ status: "error", message: "Invalid signature." }, 403);
  }
...
async function onRequest(context) {
  ...
  const db = env.DB;
  try {
    const existing = await db.prepare("SELECT id, status, coins_earned FROM offer_completions WHERE provider = ? AND transaction_id = ?").bind("primewall", transId).first();
    if (isChargeback) {
...
// ../.wrangler/tmp/pages-M7zbag/functionsRoutes-0.8631972174589038.mjs
var routes = [
  {
